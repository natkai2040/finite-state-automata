...
function getProperty(obj, propName){
    const getPropertyResponse = {found: false, value: null}; 
    if (Object.keys(obj).includes(propName)){
        getPropertyResponse.found = true; 
        getPropertyResponse.value = obj[propName]; 
    }
...
/////////////////////////////////////////
//    FSA (Finite State Automata) CLASS
/////////////////////////////////////////
class FSA {
    constructor() {
        ///////////////////////
        ...
        constructor(name) {
          ...
          this.changeDest = (oldState, newState) => {
            for (let i = 0; i < trs.length; ++i) {
              ...
              if (getProperty(trs[i], event).value === oldState) { //if it points to the old state
                let newTrs = {}; 
                setProperty(newTrs, event, newState); //create a new transition pointing to new state
          ...
          this.nextStates = (e) => {
            return (trs.filter(t => getProperty(t, e).found)).map(t => getProperty(t,e).value);
          };
        }
      }
      /////////////////////////
   ...
   //         } //if the next state exists, change curState
      ...
      this.addTransition = (s, t) => {
        ...
        let destName = getProperty(t, event).value; //gets the destination state name
